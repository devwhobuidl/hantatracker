...
import { Radio, ShieldCheck, Activity, ExternalLink } from "lucide-react";
      ...
      <div className="flex-1 flex items-center px-4 h-full overflow-hidden">
        ...
        <div className="flex-1 relative h-full flex items-center overflow-hidden group">
          <div className="flex animate-marquee whitespace-nowrap items-center hover:[animation-play-state:paused]">
            {[...(reports && reports.length > 0 ? reports : feed), ...(reports && reports.length > 0 ? reports : feed)].map((item, i) => {
                const itemType = 'type' in item ? item.type : 'INFO';
                const itemTimestamp = 'date' in item ? new Date(item.date).toLocaleTimeString() : ('timestamp' in item ? item.timestamp : '--:--');
                return (
                  <a 
                  ...
                  >
                    <span className={`font-mono text-[8px] px-2 py-0.5 rounded font-black shadow-[0_0_10px_rgba(var(--primary-rgb),0.3)] ${
                      itemType === 'CRITICAL' ? 'bg-secondary text-background glow-red' :
                      itemType === 'WARNING' ? 'bg-accent text-background' :
                      'bg-primary text-background glow-green'
                    }`}>
                      {itemType}
                    </span>
                    <span className="font-mono text-[10px] text-primary group-hover/item:text-secondary transition-colors uppercase font-bold tracking-tight">
                      {'text' in item ? item.text : item.title}
                    </span>
                    <span className="font-mono text-[8px] text-primary/40 font-bold shrink-0">
                      {isHydrated ? itemTimestamp : "--:--"}
                    </span>
                    <ExternalLink size={8} className="text-primary/20 group-hover/item:text-secondary transition-colors" />
                  </a>
                );
            })}
          </div>
